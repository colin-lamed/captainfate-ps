module Story.CaptainFate where
...
story ∷ StoryBuilder Unit
story = do

  setTitle "Captain Fate"
  setMaxScore 2
  ...
  mkObject costume $ do
    ...
    setOUse $ action $ do
      ...
      if        location == cafe   then printLn "Benny allows no monkey business in his establishment."
                ...
                if        not lightOn    then printLn "Last time you changed in the dark, you wore the suit inside out!"
                  else if toiletDoorOpen then printLn "The door to the bar stands OPEN at tens of curious eyes. You'd be forced to arrest yourself for LEWD conduct."
                                                 ...
                                                 if toiletDoorLocked then do printLn "Then you unfold your INVULNERABLE-COTTON costume and turn into Captain FATE, defender of free will, adversary of tyranny!"
...
--------------------------------------------------------------------------------
  ...
  mkRoom cafe $ do
    ...
    setRDescr $ do
      ...
      when wearingCostume $ do
        ...
        maybe (pure unit) printLn $ comments !! 0
    ...
    setRExits $ do
      ...
      exit "Street" S $ do
        ...
        let denyExit msg = do printLn "Just as you are stepping into the street, the big hand of Benny falls on your shoulder."
                              printLn msg
                              pure Nothing
        if        coffeeNeedsPaying && not bennyHasKey then denyExit "\"Hey! You've got my key and haven't paid for the coffee. Do I look like a chump?\" You apologise as only a HERO knows how to do and return inside."
          else if coffeeNeedsPaying                    then denyExit "\"Just waidda minute here, Mister,\" he says. \"Sneaking out without paying, are you?\" You quickly mumble an excuse and go back into the cafe. Benny returns to his chores with a mistrusting eye."
          else if not bennyHasKey                      then denyExit "\"Just where you think you're going with the toilet key?\" he says. \"You a thief?\" As Benny forces you back into the cafe, you quickly assure him that it was only a STUPEFYING mistake."
          else                                              pure $ Just street
    ...
    talkO $ do
      ...
      when readNoteOnce $ do
        say "Can I have the toilet key?" $ do
          ...
          if        not coffeeAskedFor then printLn "\"Toilet is only fer customers,\" he grumbles, looking pointedly at a menu board behind him."
            else if bennyHasKey        then do printLn "Benny tosses the key to the rest rooms on the counter, where you grab it with a dextrous and precise movement of your HYPER-AGILE hand."
                                               takeItem toiletKey
  ...
  let useToiletDoor = action $ do
        toiletDoorLocked ← getState toiletDoorLockedS
        toiletDoorOpen   ← getState toiletDoorOpenS
        if  toiletDoorLocked then printLn "The toilet door is locked"
  ...
  let useToiletDoorWithKey = do
        toiletDoorLocked ← getState toiletDoorLockedS
        if toiletDoorLocked then do printLn "You unlock and open the toilet door"
  ...
  mkObject toiletKey $ do
    ...
    setODescr $ do
      wearingCostume ← getState wearingCostumeS
      if wearingCostume then printLn "the used and IRRELEVANT key"
  ...
  mkObject note $ do
    ...
    setODescr $ do
      readNoteOnce ← getState readNoteOnceS
      if readNoteOnce then printLn "The scorched undecipherable note holds no SECRETS from you NOW! Ha!"
        else               do
...
--------------------------------------------------------------------------------
  ...
  mkObject lavatory $ do
    ...
    setODescr $ do
      ...
      if        not lightOn       then printLn "It's too dark to see"
        else if lavatoryFirstView then do printLn "The latest user CIVILLY flushed it after use, but failed to pick up the VALUABLE coin that fell from his pants."
                                          takeItem coin
